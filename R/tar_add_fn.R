##' Add function to functions.R
##'
##' Convenience function for adding a function template to functions.R
##'
##' @title tar_add_fn.R
##' @return Nothing. Modifies your workspace.
##' @export
tar_add_fn.R <- function(text, index) {
...
write_fn_file <- function(fn_name, fn_defn, fn_dir = "R", fn_file = "functions.R") {

  target_file <- file.path(fn_dir, fn_file)

  if (!file.exists(target_file)) {
    message(target_file, " does not exist.")
...
build_fn_body <- function(fn_name, fn_arg_list) {
  ...
  content <- getOption("tarflow_placeholder") %||% "NULL"
